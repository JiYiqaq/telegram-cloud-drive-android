...
输入 Token、私人频道 Chat ID、至少 12 位且唯一的同步密码并再次确认。建议把同步密码离线备份；丢失后任何设备都无法恢复文件。

## 上传、下载与管理

- 点击首页 `+` 可多选任意类型文件。
- 文件上传到当前虚拟目录，默认块大小可在设置调整。
- 点击可用文件后用系统创建文档界面选择保存位置。
- 首页支持多级目录、搜索、名称/大小/时间排序、重命名和移动。
- 删除会先保留所有 `message_id`，逐块删除；部分失败可重试。
...
- 文件夹批量移动当前以“移动到根目录”快速操作为主，任意目标批量选择 UI 仍可继续改进。
