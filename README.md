...
版本：`0.1.5-alpha`。核心加密、上传、下载、加密置顶索引、跨设备恢复、安全删除和网盘式本地管理流程已实现。项目仍为 alpha，尚未经过正式第三方安全审计，不应作为唯一备份。
...
- 文件、传输、设置三入口网盘式界面，上传与批量管理操作更直观
- 传输历史可安全单删或一键清理；删除记录不会删除已上传的云端文件
- 浅色、深色和跟随系统主题
- 脱敏诊断导出、清理缓存、退出并清除凭据
...
- 当前安全默认块为 19 MiB；加密块必须严格小于 20,000,000 字节下载边界。
...
- 前一个文件仍在后台删除时可以继续删除其他文件；请求会安全进入串行队列，不需要等待或重启应用。
