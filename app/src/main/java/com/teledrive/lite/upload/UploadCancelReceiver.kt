...
class UploadCancelReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action != ACTION_CANCEL_UPLOAD) return
        ...
        CoroutineScope(SupervisorJob() + Dispatchers.IO).launch {
            try {
                ...
                runCatching { application?.container?.uploadScheduler?.cancel(taskId) }
            } finally {
    ...
    companion object {
        private const val ACTION_CANCEL_UPLOAD = "com.teledrive.lite.action.CANCEL_UPLOAD"
        private const val EXTRA_TASK_ID = "task_id"

        fun pendingIntent(context: Context, taskId: String): PendingIntent {
            require(taskId.isNotBlank())
            val intent = Intent(context, UploadCancelReceiver::class.java)
